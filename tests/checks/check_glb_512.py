# WP15 golden check: the GLB's COLOR_0 must match a vectorized-numpy
# reimplementation of the flex_gemm trilinear formula (dense volume,
...
# undo the export axis swap (y,z -> z,-y): world y = -glb z, world z = glb y
world = pos.copy()
world[:, 1] = -pos[:, 2]
world[:, 2] = pos[:, 1]
q = (world - origin[None, :]) / np.float32(vs)

vol = np.zeros((GRID, GRID, GRID, attrs.shape[1]), dtype=np.float32)
occ = np.zeros((GRID, GRID, GRID), dtype=bool)
vol[coords[:, 0], coords[:, 1], coords[:, 2]] = attrs
...
for k in range(8):
    ...
    acc[sel] += w[sel, None] * vol[n[sel, 0], n[sel, 1], n[sel, 2]]
    wsum += w
...
assert abs(mat["metallicFactor"] - mm) < 1e-4, "metallic factor mismatch"
assert abs(mat["roughnessFactor"] - rr) < 1e-4, "roughness factor mismatch"
nl = np.linalg.norm(nrm, axis=1)
print("normal lengths min/max:", nl.min(), nl.max())
assert nl.max() < 1.0001 and nl.min() > 0.999, "unnormalized normals"
...
assert geom.faces.shape[0] == idx.shape[0] // 3
print("GLB golden check PASSED")
